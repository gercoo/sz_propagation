function [E,V] = spm_erp_priors_szpropagation
% prior moments for ISI forward model
% see Cooray et al in prep
% pE.sp - location of stationary point
% pE.ss - sigma of stationary point
% pE.w  - angular_speed
% pE.B  - value of B
% pE.C  - value of C
...
n       =   10;                                         % number of connectivity parameters
% parameters for neural-mass forward model
%==========================================================================
 
% set priors of model parameters
%--------------------------------------------------------------------------
E.g     =   sparse(n,1);  V.g = sparse(n,1) + 2;   % CMC1D 
E.c     =   0;         ;  V.c = 2;  
% E.ss1   = sparse(n,1);  V.ss1 = sparse(n,1) + 1/32;   % CMC1D 
% E.a1   = sparse(n,1);  V.a1 = sparse(n,1) + 1/32;   % CMC1D 
% 
% E.ss2   = sparse(n,1);  V.ss2 = sparse(n,1) + 1/32;   % CMC1D 
% E.a2   = sparse(n,1);  V.a2 = sparse(n,1) + 1/32;   % CMC1D 
% 
% E.w   = sparse(n,1);  V.w = sparse(n,1) + 1/32;   % limit cycle
% E.n   = sparse(n,1);  V.n = sparse(n,1) + 1/32;   % limit cycle
% E.A   = sparse(n,5);  V.A = sparse(n,5) + 1/16;   % limit cycle
return
